...
\caption{\label{fig:det}DFA representation of a tree transducer (above) and an input tree (middle). This transducer will transduce the input tree to the hypergraph (below) yielding a single string derivation \emph{x y z}.}
\end{figure}

To understand how this algorithm works, the input tree DFA is matched against the `S' DFA. The output transductions are stored in the final states of the transducer DFA, and when a path is matched, an edge is added to the output hypergraph, one per matched path per translation option. Variables that were used in the input are then recursively processed, starting from the relevant transducer DFA (in this case, the `A' DFA will be invoked recursively).
